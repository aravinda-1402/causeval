...
import { fixturePrompt, FixtureRunner } from "./fixture.js";
...
export function classify(
  ...
  const effect = b - m;
  if (effect < -config.pseudoCoverageCeiling) return "indeterminate";
...
export function interpret(
  ...
  switch (result.classification) {
    ...
    default:
      if (result.detectionRate < -config.pseudoCoverageCeiling)
        return `No causal conclusion: removing the instruction increased the pass rate from ${b}% to ${m}%. This inverted effect may reflect confounding or sampling variation; inspect the evidence and repeat the experiment.`;
...
export async function analyze(options: AnalyzeOptions): Promise<Report> {
  ...
  const generatedAccepted = evals.filter((e) => e.causeval?.generated).length;
  if (generatedAccepted)
    warnings.push(
      `${generatedAccepted} accepted eval(s) were generated by CausEval. Review their assertions and mapping evidence; acceptance alone is not proof of protection.`,
    );
  ...
  if (options.verify && options.runner instanceof FixtureRunner)
    options.runner.validate(evals);
  ...
  if (
    options.provider.name === "fixture" &&
    options.prompt.trim() !== fixturePrompt.trim()
  )
    warnings.push(
      "The bundled fixture provider only recognises the example prompt. New or changed rules in your prompt cannot be extracted; these results are incomplete. Set provider.type to your model (openai, anthropic, compatible or ollama) and configure a model to analyse this prompt.",
    );
